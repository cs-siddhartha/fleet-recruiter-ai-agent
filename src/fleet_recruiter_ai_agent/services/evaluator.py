...
def run_evaluation(evaluation_id: str, job: JobDetail, file_name: str, content: bytes) -> None:
    ...
    try:
        ...
        jd_analysis = analyze_jd(JDAnalysisInput(job=job), llm_client)

        evaluation_store.update_stage(evaluation_id, EvaluationStage.ANALYZING_RESUME)
        resume_analysis = analyze_resume(ResumeAnalysisInput(parsed_resume=parsed_resume), llm_client)

        evaluation_store.update_stage(evaluation_id, EvaluationStage.GENERATING_SCORECARD)
        scorecard = evaluate_scorecard(
            ScorecardEvaluationInput(job=job, jd_analysis=jd_analysis, resume_analysis=resume_analysis),
            llm_client,
        )
